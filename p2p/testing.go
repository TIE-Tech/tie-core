...
// JoinAndWait is a common method for joining a destination server
...
// LeaveAndWait is a common method for leaving a destination server
...
// constructMultiAddrs is a common function for converting raw IPs to mutliaddrs
...
// initBootnodes is a common method for specifying the server's bootnode configuration
...
func CreateServer(params *CreateServerParams) (*Server, error) {
	...
	cfg.Metrics = metrics.NewP2PMetrics()
...
// MeshJoin is a common method for joining all the passed in servers into a mesh
...
func GenerateTestLibp2pKey(t *testing.T) (crypto.PrivKey, string) {
	...
	if setErr := localSecretsManager.SetSecret(nodekey.NetworkKey, libp2pKeyEncoded); setErr != nil {
