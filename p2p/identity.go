...
func (i *identity) setup() {
	...
	i.srv.host.Network().Notify(&network.NotifyBundle{
		ConnectedF: func(net network.Network, conn network.Conn) {
			...
			logger.Debug("[IDT] identity setup Conn", "peer", peerID, "direction", conn.Stat().Direction)
...
func (i *identity) checkSlots(direction network.Direction, peerID peer.ID) (slotsFull bool) {
	...
	default:
		logger.Info("[IDT] identity checkSlots Invalid connection direction", "peer", peerID)
	}
...
func (i *identity) Bye(ctx context.Context, req *proto.ByeMsg) (*empty.Empty, error) {
	...
	logger.Debug("[IDT] identity peer bye", "id", connContext.PeerID, "msg", req.Reason)
