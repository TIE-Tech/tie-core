...
func DefaultConfig() *Config {
	return &Config{
		NoDiscover:       false,
		Addr:             &net.TCPAddr{IP: net.ParseIP("127.0.0.1"), Port: types.DefaultLibp2pPort},
		MaxPeers:         40,
		MaxInboundPeers:  32,
		MaxOutboundPeers: 8,
	}
...
// setupLibp2pKey is a common method for setting up the networking private key
func setupLibp2pKey(secretsManager nodekey.SecretsManager) (crypto.PrivKey, error) {
	var key crypto.PrivKey

	if secretsManager.HasSecret(nodekey.NetworkKey) {
...
func (s *Server) Start() error {
	...
	logger.Info("[SVR] libp2p server running", "addr", AddrInfoToString(s.AddrInfo()))
	...
	if !s.config.NoDiscover {
		...
		for _, raw := range s.config.Chain.Bootnodes {
			...
			if node.ID == s.host.ID() {
				logger.Info("[SVR] omitting bootnode with same ID as host", "id", node.ID)
				continue
			}
	...
	go func() {
		if err := s.runJoinWatcher(); err != nil {
			logger.Error(fmt.Sprintf("Unable to start join watcher service, %v", err))
		}
	}()
...
func (s *Server) runDial() {
	...
	if err != nil {
		logger.Error("dial manager failed to subscribe", "err", err)
	}
	...
	for {
		for i := int64(0); i < s.numOpenSlots(); i++ {
			...
			logger.Debug("[SVR] dial", "local", s.host.ID(), "addr", tt.addr.String())
			...
			if s.isConnected(tt.addr.ID) {
				...
				if err = s.host.Connect(context.Background(), *tt.addr); err != nil {
					addrs := fmt.Sprintf("%v", tt.addr.Addrs)
					logger.Error("runDial host.Connect failed", "addr", addrs, "err", err)
...
func (s *Server) addPeer(id peer.ID, direction network.Direction) {
	logger.Info("[SVR] Peer connected", "id", id.String())
	s.peersLock.Lock()
	defer s.peersLock.Unlock()
...
func (s *Server) delPeer(id peer.ID) {
	logger.Info("[SVR] Peer disconnected", "id", id.String())
...
func (s *Server) Disconnect(peer peer.ID, reason string) {
	if s.host.Network().Connectedness(peer) == network.Connected {
		logger.Info(fmt.Sprintf("Closing connection to peer [%s] for reason [%s]", peer.String(), reason))

		if closeErr := s.host.Network().ClosePeer(peer); closeErr != nil {
			logger.Error(fmt.Sprintf("Unable to gracefully close peer connection, %v", closeErr))
		}
	}
}
...
func (s *Server) Join(addr *peer.AddrInfo, timeout time.Duration) error {
	logger.Info("[SVR] Join request", "addr", addr.String())
...
func (s *Server) wrapStream(id string, handle func(network.Stream)) {
	s.host.SetStreamHandler(protocol.ID(id), func(stream network.Stream) {
		...
		logger.Debug("[SVR] open stream", "protocol", id, "peer", peerID)
...
func (s *Server) emitEvent(peerID peer.ID, typ PeerEventType) {
	...
	if err := s.emitterPeerEvent.Emit(evnt); err != nil {
		logger.Info("failed to emit event", "peer", evnt.PeerID, "type", evnt.Type, "err", err)
	}
}
...
// SubscribeFn is a common method to run subscription of PeerEvents
