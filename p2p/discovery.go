...
func (d *discovery) setup(bootnodes []*peer.AddrInfo) error {
	...
	err = d.srv.SubscribeFn(func(evnt *PeerEvent) {
		...
		case PeerConnected:
			...
			if err != nil {
				logger.Error("failed to add peer to routing table", "err", err)
				return
			}
			d.peers.add(peerID)
...
func (d *discovery) setupTable() {
	for _, node := range d.bootnodes {
		if err := d.addToTable(node); err != nil {
			logger.Error("Failed to add new peer to routing table", "peer", node.ID, "err", err)
		}
	}
}

func (d *discovery) attemptToFindPeers(peerID peer.ID) error {
	logger.Debug("[DSC] Querying a peer for near peers", "peer", peerID)
	...
	logger.Debug("[DSC] Found new near peers", "peer", len(nodes))
	...
	if d.srv.numOpenSlots() > 0 {
		if target := d.peers.getRandomPeer(); target != nil {
			if err := d.attemptToFindPeers(target.id); err != nil {
				logger.Error("failed to dial peer", "peer", target.id, "err", err)
			}
		}
	}
}
