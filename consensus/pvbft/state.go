...
func (c *currentState) NumValid() int {
	...
	return 2 * c.vset.MaxFaultyNodes()
}
...
func (c *currentState) CalcProposerPoa(lastProposer types.Address) {
	c.proposer = c.vset.CalcProposerPoa(c.view.Round, lastProposer)
}

// CalcProposer calculates the proposer and sets it to the state
func (c *currentState) CalcProposer(randInt *big.Int) {
	c.proposer = c.vset.CalcProposer(randInt.Uint64())
}
