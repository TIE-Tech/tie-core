...
func ecrecoverImpl(sig, msg []byte) (*ecdsa.PublicKey, error) {
	return crypto.RecoverPubkey(sig, crypto.Keccak256(msg))
}

func ecrecoverFromHeader(h *types.Header) (*ecdsa.PublicKey, error) {
...
func WriteSeal(prv *ecdsa.PrivateKey, h *types.Header, vrfData []byte) (*types.Header, error) {
	return writeSeal(prv, h, vrfData)
}
...
func writeSeal(prv *ecdsa.PrivateKey, h *types.Header, vrfData []byte) (*types.Header, error) {
	...
	// TODO 2 writeSeal save
	vrfValue, vrfProof, err := vrf.Vrf(prv, vrfData)
	if err != nil {
		return nil, err
	}
	extra.SetVrfInfo(vrfValue, vrfProof)
...
func writeCommittedSeals(h *types.Header, seals [][]byte) (*types.Header, error) {
	...
	extra, err := getIbftExtra(h)
	if err != nil {
		return nil, errors.New("getIbftExtra err:" + err.Error())
	}
	extra.CommittedSeal = seals

	if err = PutIbftExtra(h, extra); err != nil {
		return nil, errors.New("PutIbftExtra err:" + err.Error())
	}
	return h, nil
}
...
func verifySigner(snap *Snapshot, header *types.Header) (*ecdsa.PublicKey, error) {
	signerPub, err := ecrecoverFromHeader(header)
	if err != nil {
		return nil, err
	}

	signer := crypto.PubKeyToAddress(signerPub)
	if !snap.Includes(signer) {
		return nil, fmt.Errorf("not found signer")
	}
...
func verifyCommitedFields(snap *Snapshot, header *types.Header) error {
	...
	for _, seal := range extra.CommittedSeal {
		pub, err := ecrecoverImpl(seal, rawMsg)
		if err != nil {
			return err
		}
	...
	// 	+1	is the proposer
	if validSeals := len(visited); validSeals <= 2*snap.MaxFaultyNodes() {
...
func validateMsg(msg *proto.MessageReq) error {
	...
	pub, err := ecrecoverImpl(buf, signMsg)
	if err != nil {
		return err
	}
...
func signMsg(key *ecdsa.PrivateKey, msg *proto.MessageReq) error {
	sign, err := msg.PayloadNoSig()
	if err != nil {
		return err
	}

	sig, err := crypto.Sign(key, crypto.Keccak256(sign))
	if err != nil {
		return err
	}
