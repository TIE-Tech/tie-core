...
func (ap *testerAccountPool) genesis() *params.Genesis {
	genesis := &types.Header{
		MixHash: IstanbulDigest,
	}
	validators := ap.ValidatorSet()
	putIbftExtraValidators(genesis, validators)
	genesis.ComputeHash()
...
func (ap *testerAccountPool) ValidatorSet() []types.Address {
	validators := make([]types.Address, 0)
	for _, i := range ap.accounts {
		validators = append(validators, i.Address())
	}
	return validators
}
...
func buildHeaders(pool *testerAccountPool, genesis *params.Genesis, mockHeaders []mockHeader) []*types.Header {
...
func TestSnapshot_setupSnapshot(t *testing.T) {
	...
	validatorSet.SetValidators(pool.ValidatorSet())
	...
	var cases = []struct {
		...
		},
		{
			...
			savedSnapshots: []*Snapshot{
				newSnapshot(0, NewValidatorSet(), []*Vote{}),
			},
			expectedResult: snapshotData{
				LastBlock: 2,
				Snapshots: []*Snapshot{
			...
			for indx, header := range headers {
				...
				if result != nil {
					...
					for _, i := range result.validators {
						resSnap.Set = append(resSnap.Set, pool.get(i).Address())
					}
					// build result votes
