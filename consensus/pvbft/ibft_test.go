...
func TestTransition_AcceptState_Validator_VerifyCorrect(t *testing.T) {
	...
	block := i.DummyBlock()
	header, err := writeSeal(i.pool.get("A").priv, block.Header, nil)
...
func (s *mockSyncer) BestPeer() *syncer.SyncPeer {
	return &syncer.SyncPeer{}
}

func (s *mockSyncer) BulkSyncWithPeer(p *syncer.SyncPeer, handler func(block *types.Block)) error {
...
func (s *mockSyncer) WatchSyncWithPeer(p *syncer.SyncPeer, handler func(b *types.Block) bool) {
...
func newMockIbft(t *testing.T, accounts []string, account string) *mockIbft {
	...
	ibft.state.vset = NewValidatorSet()
