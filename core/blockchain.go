...
type Blockchain struct {
	...
	config  *params.Chain // Config containing chain information
...
func NewBlockchain(
	...
	if dataDir == "" {
		if db, err = memory.NewMemoryStorage(); err != nil {
			return nil, err
		}
	} else {
		if db, err = leveldb.NewLevelDBStorage(filepath.Join(dataDir, "blockchain")); err != nil {
			return nil, err
		}
	}
...
func (b *Blockchain) ComputeGenesis() error {
	...
	if ok {
		...
		logger.Info("[BLK] ComputeGenesis Current header", "hash", header.Hash.String(), "number", header.Number)
	...
	logger.Info("[BLK] ComputeGenesis genesis", "hash", b.config.Genesis.Hash())
...
func (b *Blockchain) Config() *params.Params {
...
func (b *Blockchain) writeGenesis(genesis *params.Genesis) error {
...
func (b *Blockchain) WriteBlock(block *types.Block) error {
	...
	// TODO WriteBlock Verify the header
	if err := b.consensus.VerifyHeader(parent, block.Header); err != nil {
		return fmt.Errorf("failed to verify the header: %w", err)
	}

	// Verify body data
	if hash := calcroot.CalculateUncleRoot(block.Uncles); hash != block.Header.Sha3Uncles {
		return fmt.Errorf(
	...
	if hash := calcroot.CalculateTransactionsRoot(block.Transactions); hash != block.Header.TxRoot {
		return fmt.Errorf(
	...
	logArgs := []interface{}{"block", header.Number, "txns", len(block.Transactions)}

	if prevHeader, ok := b.GetHeaderByNumber(header.Number - 1); ok {
		diff := header.Timestamp - prevHeader.Timestamp
		logArgs = append(logArgs, "time", diff)
	}

	logger.Info("[BLK] write block success", logArgs...)
	return nil
}
...
func (b *Blockchain) processBlock(block *types.Block) (*state.BlockResult, error) {
	...
	receiptSha := calcroot.CalculateReceiptsRoot(result.Receipts)
...
// verifyGasLimit is a common function for validating a gas limit in a header
