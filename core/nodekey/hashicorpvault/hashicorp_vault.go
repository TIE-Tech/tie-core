...
// constructSecretPath is a common method for constructing a path to the secret
...
func (v *VaultSecretsManager) SetSecret(name string, value []byte) error {
	...
	if err == nil {
		// Secret is present
		logger.Warn(fmt.Sprintf("Overwriting secret: %s", name))
	} else if !errors.Is(err, nodekey.ErrSecretNotFound) {
