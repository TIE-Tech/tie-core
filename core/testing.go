...
func NewTestBodyChain(n int) ([]*types.Header, []*types.Block, [][]*types.Receipt) {
	...
	for i := 1; i < n; i++ {
		...
		header.ReceiptsRoot = calcroot.CalculateReceiptsRoot(localReceipts)
...
func NewTestBlockchain(t *testing.T, headers []*types.Header) *Blockchain {
	...
	config := &params.Chain{
		...
		Params: &params.Params{
			Forks: &params.Forks{
				EIP155:    params.NewFork(0),
				Homestead: params.NewFork(0),
			},
	...
	// find a way to add the snapshot, this will fail until that is fixed.
...
func TestBlockchain(t *testing.T, genesis *params.Genesis) *Blockchain {
	if genesis == nil {
		genesis = &params.Genesis{}
	}
...
func newBlockChain(config *params.Chain, executor Executor) (*Blockchain, error) {
	if executor == nil {
		executor = &mockExecutor{}
	}

	b, err := NewBlockchain("", config, &MockVerifier{}, executor)
	if err != nil {
		return nil, err
	}
