...
var (
	defaultNetworkConfig = func(c *p2p.Config) {
		c.NoDiscover = true
	}
)
...
func CreateSyncer(t *testing.T, blockchain blockchainShim, serverCfg *func(c *p2p.Config)) *Syncer {
	t.Helper()
	...
	srv, createErr := p2p.CreateServer(&p2p.CreateServerParams{
	...
	syncer := NewSyncer(srv, blockchain)
	syncer.Start()
...
func SetupSyncerNetwork(
	...
	for idx, peerChain := range peerChains {
		...
		if joinErr := p2p.JoinAndWait(
			syncer.server,
			peerSyncers[idx].server,
			p2p.DefaultBufferTimeout,
			p2p.DefaultJoinTimeout,
		); joinErr != nil {
