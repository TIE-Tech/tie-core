...
// createNetworkServers is a common function for generating p2p servers
func createNetworkServers(t *testing.T, count int, conf func(c *p2p.Config)) []*p2p.Server {
	t.Helper()

	networkServers := make([]*p2p.Server, count)

	for indx := 0; indx < count; indx++ {
		server, createErr := p2p.CreateServer(&p2p.CreateServerParams{ConfigCallback: conf})
		if createErr != nil {
			t.Fatalf("Unable to create p2p servers, %v", createErr)
		}
...
// createSyncers is a common function for generating syncers. Servers and BlockStores should be at least the length
// of count
func createSyncers(count int, servers []*p2p.Server, blockStores []*mockBlockStore) []*Syncer {
	syncers := make([]*Syncer, count)

	for indx := 0; indx < count; indx++ {
		syncers[indx] = NewSyncer(servers[indx], blockStores[indx])
	}
...
func TestSyncer_PeerDisconnected(t *testing.T) {
	...
	joinErrors := p2p.MeshJoin(servers...)
	if len(joinErrors) != 0 {
		t.Fatalf("Unable to join servers [%d], %v", len(joinErrors), joinErrors)
	}

	// wait until gossip protocol builds the mesh p2p
