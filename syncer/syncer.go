...
import (
	...
	libp2pGrpc "github.com/TIE-Tech/tie-core/p2p/grpc"
...
func NewSyncer(server *p2p.Server, blockchain blockchainShim) *Syncer {
	s := &Syncer{
...
func (s *Syncer) enqueueBlock(peerID peer.ID, b *types.Block) {
	logger.Debug("[SYN] enqueue block", "peer", peerID, "number", b.Number(), "hash", b.Hash())

	peer, ok := s.peers.Load(peerID)
	if ok {
		syncPeer, ok := peer.(*SyncPeer)
		if !ok {
			logger.Error("invalid sync peer type cast")
			return
		}
...
func (s *Syncer) Broadcast(b *types.Block) {
	...
	if !ok {
		...
		logger.Error("total difficulty not found", "block number", b.Number())
		return
	}
...
func (s *Syncer) Start() {
	s.serviceV1 = &serviceV1{syncer: s, store: s.blockchain}
...
func (s *Syncer) setupPeers() {
	for _, p := range s.server.Peers() {
		if addErr := s.AddPeer(p.Info.ID); addErr != nil {
			logger.Error("Error when adding peer", "peerId", p.Info.ID, "err", addErr)
		}
	}
}

// handlePeerEvent subscribes p2p event and adds/deletes peer from syncer
		...
		for {
			...
			case p2p.PeerConnected:
				if err := s.AddPeer(evnt.PeerID); err != nil {
					logger.Error("failed to add peer", "err", err)
				}
...
func (s *Syncer) WatchSyncWithPeer(p *SyncPeer, handler func(b *types.Block) bool) {
	...
	for {
		if p.IsClosed() {
			logger.Warn("[SYN] Connection closed already", "id", p.peer)
			break
		}
...
func (s *Syncer) logSyncPeerPopBlockError(err error, peer *SyncPeer) {
	if errors.Is(err, ErrPopTimeout) {
		...
		logger.Warn(fmt.Sprintf(msg, int(popTimeout.Seconds()), peer.peer))
	} else {
		logger.Info("[SYN] failed to pop block from peer", "id", peer.peer, "err", err)
	}
}
...
func (s *Syncer) BulkSyncWithPeer(p *SyncPeer, newBlockHandler func(block *types.Block)) error {
	...
	// find in batches
	logger.Debug("[SYN] fork found", "ancestor", ancestor.Number)
	...
	for {
		...
		for {
			logger.Debug("[SYN] sync up to block", "from", startBlock.Number, "to", target)
