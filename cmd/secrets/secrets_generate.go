...
func (s *SecretsGenerate) DefineFlags() {
	...
	s.FlagMap["type"] = helper.FlagDescriptor{
		Description: fmt.Sprintf("Specifies the type of the secrets manager. Default: %s", nodekey.HashicorpVault),
		Arguments: []string{
			"TYPE",
		},
		ArgumentsOptional: false,
...
func (s *SecretsGenerate) Run(args []string) int {
	...
	flags.StringVar(&serviceType, "type", string(nodekey.HashicorpVault), "")
	...
	if !nodekey.SupportedServiceManager(nodekey.SecretsManagerType(serviceType)) {
	...
	config := &nodekey.SecretsManagerConfig{
		Token:     token,
		ServerURL: serverURL,
		Type:      nodekey.SecretsManagerType(serviceType),
		Name:      name,
		Namespace: namespace,
		Extra:     nil,
	}
