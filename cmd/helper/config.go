...
type Config struct {
	...
	EsOwner        string                 `json:"es_owner"`
	EsOpen         bool                   `json:"es_open"`
...
// Network defines the p2p configuration params
type Network struct {
...
func (c *Config) BuildConfig() (*server.Config, error) {
	...
	// Decode the chain
	cc, err := params.ImportFromFile(c.Chain)
	if err != nil {
		return nil, err
	}
	...
	if c.Secrets != "" {
		secretsConfig, readErr := nodekey.ReadConfig(c.Secrets)
		if readErr != nil {
	...
	if c.BlockTime != types.DefaultBlockTime {
...
func (c *Config) mergeConfigWith(otherConfig *Config) error {
	...
	if otherConfig.LogPath != "" {
		c.LogPath = otherConfig.LogPath
	}
	...
	if otherConfig.BlockTime != types.DefaultBlockTime {
		c.BlockTime = otherConfig.BlockTime
	}

	// elastic config
	if otherConfig.EsOpen != false {
		c.EsOpen = otherConfig.EsOpen
	}
	if otherConfig.EsAddr != "" {
		c.EsAddr = otherConfig.EsAddr
	}
	if otherConfig.EsIndex != "" {
		c.EsIndex = otherConfig.EsIndex
	}
	if otherConfig.EsOwner != "" {
		c.EsOwner = otherConfig.EsOwner
	}
