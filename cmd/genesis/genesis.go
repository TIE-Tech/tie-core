...
func (c *GenesisCommand) DefineFlags() {
	...
	c.FlagMap["dir"] = helper.FlagDescriptor{
		Description: fmt.Sprintf("Sets the directory for the TIE genesis data. Default: %s", helper.GenesisFileName),
		Arguments: []string{
			"DIRECTORY",
		},
	...
	c.FlagMap["epoch-size"] = helper.FlagDescriptor{
		Description: fmt.Sprintf("Sets the epoch size for the chain. Default %d", types.DefaultEpochSize),
		Arguments: []string{
			"EPOCH_SIZE",
		},
		ArgumentsOptional: false,
...
func (c *GenesisCommand) Run(args []string) int {
	...
	flags.Uint64Var(&epochSize, "epoch-size", types.DefaultEpochSize, "")
	...
	if consensus == ibftConsensus {
		...
		extraData = make([]byte, pvbft.IstanbulExtraVanity)
	...
	// constructEngineConfig is a common method for
	...
	cc := &params.Chain{
		...
		Genesis: &params.Genesis{
			...
			Difficulty: 1,
			Alloc:      map[types.Address]*params.GenesisAccount{},
			ExtraData:  extraData,
			GasUsed:    helper.GenesisGasUsed,
		},
		Params: &params.Params{
			ChainID: int(chainID),
			Forks:   params.AllForksEnabled,
	...
	if isPos && (consensus == ibftConsensus) {
...
func readValidatorsByRegexp(prefix string) ([]types.Address, error) {
	...
	for _, file := range files {
		...
		possibleConsensusPath := filepath.Join(path, "consensus", pvbft.IbftKeyName)
