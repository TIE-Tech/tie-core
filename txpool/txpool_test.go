...
var (
	forks = &params.Forks{
		...
		Istanbul:  params.NewFork(0),
	}

	newMetrics = metrics.NewTxMetrics()
)
...
func TestAddTxErrors(t *testing.T) {
	setupPool := func(header *types.Header) *TxPool {
		...
		pool.EnableDev()
		pool.SetSigner(state.NewEIP155Signer(100))
...
func TestAddGossipTx(t *testing.T) {
	...
	signer := state.NewEIP155Signer(uint64(100))
	tx := newTx(addr1, 1, 1)
...
func TestRecovery(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			// common callback for transition errors
