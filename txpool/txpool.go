...
// store interface defines State common methods the Txpool should have access to
...
func NewTxPool(
	...
	pool.eventManager = newEventManager()
...
// and broadcasts it to the p2p (if enabled).
func (p *TxPool) AddTx(tx *types.Transaction) error {

	if err := p.addTx(local, tx); err != nil {
		logger.Error("failed to add tx", "err", err)
		return err
	}

	// broadcast the transaction only if p2p is enabled
	...
	if p.topic != nil && !p.dev {
		...
		if err := p.topic.Publish(tx); err != nil {
			logger.Error("failed to topic tx", "err", err)
		}
	}
...
func (p *TxPool) Drop(tx *types.Transaction) {
	...
	logger.Debug("[TXP] dropped account txs",
		"num", droppedCount,
...
func (p *TxPool) processEvent(event *blockchain.Event) {
	...
	for _, header := range event.NewChain {
		block, ok := p.store.GetBlockByHash(header.Hash, true)
		if !ok {
			logger.Error("could not find block in store", "hash", header.Hash.String())
...
func (p *TxPool) addTx(origin txOrigin, tx *types.Transaction) error {
	...
	logger.Debug("[TXP] add tx", "origin", origin.String(), "hash", tx.Hash.String())
	...
	if _, ok := p.index.get(tx.Hash); ok {
		if origin == gossip {
			...
			logger.Debug("[TXP] dropping known gossiped transaction", "hash", tx.Hash.String())
			return nil
		} else {
	...
	logger.Debug("[TXP] add tx to enqueueReqCh success", "hash", tx.Hash)
	return nil
}
...
func (p *TxPool) handleEnqueueRequest(req enqueueRequest) {
	...
	if err := account.enqueue(tx); err != nil {
		logger.Error("enqueue error", "err", err)
		return
	}

	logger.Debug("[TXP] enqueue request", "hash", tx.Hash.String())
	...
	if tx.Nonce > account.getNonce() {
		logger.Warn("[TXP] handleEnqueueRequest nonce too large", "hash", tx.Hash, "nonce", tx.Nonce)
		return
	}

	p.promoteReqCh <- promoteRequest{account: addr} // BLOCKING
	logger.Debug("[TXP] add tx to promoteReqCh success", "hash", tx.Hash)
}
...
func (p *TxPool) handlePromoteRequest(req promoteRequest) {
	...
	logger.Debug("[TXP] promote request", "promoted", promoted, "addr", addr.String())
}
...
func (p *TxPool) addGossipTx(obj interface{}) {
	...
	if err := tx.UnmarshalRLP(raw.Raw.Value); err != nil {
		logger.Error("failed to decode broadcasted tx", "err", err)
		return
	}

	// add tx
	if err := p.addTx(gossip, tx); err != nil {
		logger.Error("failed to add broadcasted txn", "err", err)
	}
}
