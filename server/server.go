...
import (
	...
	"github.com/TIE-Tech/tie-core/rpc"
	...
	"github.com/TIE-Tech/tie-core/tievm/evm/execute"
	"github.com/TIE-Tech/tie-core/tievm/evm/precompiled"
...
func NewServer(config *Config) (*Server, error) {
	...
	logger.Info("[SVR] NewServer Data dir", "path", config.DataDir)
	...
	if config.Telemetry.PrometheusAddr != nil {
		m.serverMetrics = metrics.MetricProvider("tiechain", config.Chain.Name, true)
		m.prometheusServer = m.startPrometheusServer(config.Telemetry.PrometheusAddr)
	} else {
		m.serverMetrics = metrics.MetricProvider("tiechain", config.Chain.Name, false)
	}
	...
	// start libp2p
	{
		...
		network, err := p2p.NewServer(netConfig)
		if err != nil {
			return nil, err
		}
		m.network = network
	...
	stateStorage, err := itrie.NewLevelDBStorage(filepath.Join(m.config.DataDir, "trie"))
	if err != nil {
		return nil, err
	}
	...
	m.executor.SetRuntime(execute.NewEVM())
	...
	// blockchain object
	m.blockchain, err = blockchain.NewBlockchain(m.config.DataDir, config.Chain, nil, m.executor)
	if err != nil {
		return nil, err
	}
...

	{
		...
		signer := state.NewEIP155Signer(uint64(m.config.Chain.Params.ChainID))
...
func (s *Server) setupSecretsManager() error {
	...
	if secretsManagerConfig == nil {
		...
		secretsManagerConfig = &nodekey.SecretsManagerConfig{
			Type: nodekey.Local,
		}
	}

	secretsManagerType := secretsManagerConfig.Type
	secretsManagerParams := &nodekey.SecretsManagerParams{}
	if secretsManagerType == nodekey.Local {
		// Only the base directory is required for
		// the local secrets manager
		secretsManagerParams.Extra = map[string]interface{}{
			nodekey.Path: s.config.DataDir,
		}
	}
...
func (j *jsonRPCHub) getState(root types.Hash, slot []byte) ([]byte, error) {
	...
	if !ok {
		return nil, rpc.ErrStateNotFound
	}
...
func (j *jsonRPCHub) GetForksInTime(blockNumber uint64) params.ForksInTime {
...
) (result *evm.ExecutionResult, err error) {
...
func (s *Server) setupGRPC() error {
	...
	go func() {
		if err = s.grpcServer.Serve(lis); err != nil {
			logger.Error("grpcServer.Serve err", "err", err)
		}
	}()

	logger.Info("[SVR] grpc server running", "addr", s.config.GRPCAddr.String())
	return nil
}

// Chain returns the chain object of the client
func (s *Server) Chain() *params.Chain {
	return s.chain
}
...
func (s *Server) Close() {
	...
	if err := s.blockchain.Close(); err != nil {
		logger.Error("failed to close blockchain", "err", err.Error())
	}

	// Close the networking layer
	if err := s.network.Close(); err != nil {
		logger.Error("failed to close networking", "err", err.Error())
	}

	// Close the consensus layer
	if err := s.consensus.Close(); err != nil {
		logger.Error("failed to close consensus", "err", err.Error())
	}

	// Close the state storage
	if err := s.stateStorage.Close(); err != nil {
		logger.Error("failed to close storage for trie", "err", err.Error())
	}
...
// SetupDataDir sets up the tie data directory and sub-folders
...
func (s *Server) startPrometheusServer(listenAddr *net.TCPAddr) *http.Server {
	...
	go func() {
		logger.Info("[SVR] Prometheus server started", "addr=", listenAddr.String())
