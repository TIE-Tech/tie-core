...
func NewFilterManager(store filterManagerStore) *FilterManager {
	m := &FilterManager{
		store:       store,
	...
	for {
		...
		case evnt := <-watchCh:
			...
			if err := f.dispatchEvent(evnt); err != nil {
				logger.Error("failed to dispatch event", "err", err)
			}

		case <-timeoutCh:
			// timeout for filter
			if !f.Uninstall(filter.id) {
				logger.Error("failed to uninstall filter", "id", filter.id)
			}
...
func (f *FilterManager) dispatchEvent(evnt *blockchain.Event) error {
	...
	for _, i := range evnt.OldChain {
		if processErr := processBlock(i, true); processErr != nil {
			logger.Error("Unable to process block", "err", processErr)
		}
	}
	// process new chain
	...
	for _, filter := range f.filters {
		if filter.isWS() {
			if flushErr := filter.flush(); flushErr != nil {
				logger.Error("Unable to process flush", "err", flushErr)
			}
		}
	}
